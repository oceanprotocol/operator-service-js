...
computeRoutes.get('/compute', async (req, res) => {
  try {
    const data: GetComputeJobStatusQuery = req.body ?? req.query // Check for both body and query params
    ...
    const agreementId = data.agreementId?.length >= 2 ? data.agreementId : undefined
    const owner = data.owner?.length >= 2 ? data.owner : undefined
    const jobId = data.jobId?.length >= 2 ? data.jobId : undefined
    const chainId = data.chainId

    // Verify provider signature if jobId is provided
    if (jobId) {
      const signMessage = `${owner}${jobId}`
      const { message, statusCode, address } = await processProviderSignatureValidation(
    ...
    // Call your function to get job status (replace with your implementation)
  ...
  try {
    ...
    if (!workflow) {
      return res.status(400).json({
        error: '`workflow` is required in the payload and must include workflow stages'
      })
    }
    ...
    if (!checkEnvironmentExists(environment, workflow.chainId)) {
      ...
      return res.status(400).json({ error: 'Environment invalid or does not exist' })
    }
    ...
    if (stages.length > 1) {
      ...
      return res.status(400).json({ error: 'Multiple stages are not supported yet' })
    }

    for (const attribute of ['algorithm', 'compute', 'input', 'output']) {
      if (!stages[0][attribute]) {
        console.error(`Missing ${attribute} in stage 0`)
        return res.status(400).json({ error: `Missing ${attribute} in stage 0` })
      }
    }
    ...
    await createSqlJob(agreementId, jobId.toString(), owner, body, environment, address)
...
computeRoutes.put('/compute', async (req, res) => {
  try {
    const data: StopComputeJobBody = req.body || req.query // Check for both body and query params
    ...
    if (agreementId?.length < 2 && agreementId) {
      agreementId = null // Ignore invalid agreementId
    }

    if (jobId?.length < 2 && jobId) {
      jobId = null // Ignore invalid jobId
    }
    ...
    const {
      ...
      statusCode,
      address
    } = await processProviderSignatureValidation(providerSignature, signMessage, nonce)
    if (message) {
...
computeRoutes.delete('/compute', async (req, res) => {
  try {
    ...
    return res.status(200).send('Operator engine handles this, try to call that endpoint')
  } catch (error) {
    console.log(LOG_LEVELS_STR.LEVEL_ERROR, `Error: ${error}`)
    res.status(500).send('Internal Server Error')
  }
